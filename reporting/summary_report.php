...
if (!in_array($_SESSION['role_id'], $allowedRoles)) {
  echo "<p>У вас немає доступу до перегляду підсумкового звіту.</p>";
  exit();
}
...
// Отримання підсумкового звіту
$summaryReport = $reportController->getSummaryReport();
?>
...
<section class="pt-5 pb-5">
  <div class="container">
    <h1 class="text-center">Підсумковий звіт</h1>
          ...
          <table class="table table-striped table-condensed table-hover"
            ...
            <tr>
              ...
              <th data-sortable="true" class="text-center" title="Видані бюлетені" data-filter-control="input"
                  data-visible="true"
                  data-field="total_ballots_issued">Видані бюлетені
              </th>
              ...
              <th data-sortable="true" class="text-center" title="Провідний кандидат" data-filter-control="input"
                  data-visible="true"
                  data-field="leading_candidate">Провідний кандидат
              </th>
              <th data-sortable="true" class="text-center" title="Максимальна кількість голосів"
                  data-filter-control="input"
                  data-visible="true"
                  data-field="max_votes">Максимальна кількість голосів
              </th>
            </tr>
            </thead>
            <tbody>
            <?php foreach ($summaryReport as $result): ?>
              <tr id="tr-id-<?= htmlspecialchars($result['summary_id']); ?>"
                ...
                <td title="<?= htmlspecialchars($result['total_ballots_issued']); ?>">
                    <span data-type="text" data-title="Видані бюлетені" data-mode="popup" data-placement="top"
                          data-pk="<?= htmlspecialchars($result['summary_id']); ?>" data-name="total_ballots_issued">
                      <?= htmlspecialchars($result['total_ballots_issued']); ?> </span>
                </td>
                ...
                <td title="<?= htmlspecialchars($result['leading_candidate'] ?? 'Немає даних'); ?>">
                    <span data-type="text" data-title="Провідний кандидат" data-mode="popup" data-placement="top"
                ...
                <td title="<?= htmlspecialchars($result['max_votes'] ?? 'Немає даних'); ?>">
                    <span data-type="text" data-title="Максимальна кількість голосів" data-mode="popup"
                          data-placement="top"
                          data-pk="<?= htmlspecialchars($result['summary_id']); ?>" data-name="max_votes">
                      <?= htmlspecialchars($result['max_votes'] ?? 'Немає даних'); ?></span>
                </td>
              </tr>
            <?php endforeach; ?>
            </tbody>
  ...
  $(document).ready(function () {
    $(document).on("click", "a[data-type='pdf']", function () {
      ...
      pdf.addHTML($("#table111"), function () {
        pdf.save('Результати по дільницях.pdf');
