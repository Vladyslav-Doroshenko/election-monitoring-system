...
if (!in_array($_SESSION['role_id'], $allowedRoles)) {
  echo "<p>У вас немає доступу до перегляду результатів явки виборців.</p>";
  exit();
}
...
// Отримання даних про явку для відображення
...
<head>
  ...
  <title>Підрахунок явки виборців</title>
...
<section class="pt-5 pb-5">
  <div class="container">
    <h1 class="text-center">Явка виборців по дільницях</h1>

    <div class="row">
      <div class="w-100">
        <!-- Таблиця з результатами явки -->
        <h2 class="h3 text-center mb-3">Результати явки по дільницях</h2>
        ...
        <div id="table1" class="table-responsive">
          <table class="table table-striped table-condensed table-hover"
                 ...
                 data-export-options='{ "fileName":"Результати по дільницях", "worksheetName":"list1" }'>
            <thead>
            <tr>
            ...
            <tbody>
            <?php foreach ($turnoutResults as $result): ?>
              <tr id="tr-id-<?= htmlspecialchars($result['station_id']); ?>"
                  class="tr-c-<?= htmlspecialchars($result['station_id']); ?>">
                <td id="td-id-<?= htmlspecialchars($result['station_id']); ?>"
                    class="td-c-<?= htmlspecialchars($result['station_id']); ?>"></td>
                <td title="<?= htmlspecialchars($result['station_name']); ?>">
                    <span data-type="text" data-title="Дільниця" data-mode="popup" data-placement="top"
                          data-pk="<?= htmlspecialchars($result['station_id']); ?>" data-name="station_name">
                      <?= htmlspecialchars($result['station_name']); ?> </span>
                </td>
                <td title="<?= htmlspecialchars($result['total_voters']); ?>">
                    <span data-type="text" data-title="Загальна кількість виборців" data-mode="popup"
                          data-placement="top"
                          data-pk="<?= htmlspecialchars($result['station_id']); ?>" data-name="total_voters">
                      <?= htmlspecialchars($result['total_voters']); ?> </span>
                </td>
                <td title="<?= htmlspecialchars($result['ballots_issued']); ?>">
                    <span data-type="text" data-title="Видані бюлетені" data-mode="popup" data-placement="top"
                          data-pk="<?= htmlspecialchars($result['station_id']); ?>" data-name="ballots_issued">
                      <?= htmlspecialchars($result['ballots_issued']); ?> </span>
                </td>
                <td title="<?= htmlspecialchars(number_format($result['voter_turnout'], 2)); ?>%">
                    <span data-type="text" data-title="Явка виборців (%)" data-mode="popup" data-placement="top"
                          data-pk="<?= htmlspecialchars($result['station_id']); ?>" data-name="voter_turnout">
